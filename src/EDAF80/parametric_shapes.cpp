                              ...
                              unsigned int const vertical_split_count)
{
	...
	glBufferData(GL_ARRAY_BUFFER, 64u, // Why 64? 32 bytes per triangle?
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, 256u,
	             /* where is the data stored on the CPU? */index_sets.data(),
	             /* inform OpenGL that the data is modified once, but used often */GL_STATIC_DRAW);

	data.indices_nb = 6u; // Two triangles?
                                ...
                                unsigned int const latitude_split_count)
{
	...
	for (unsigned int i = 0u; i < longitude_vertices_count; ++i) {
		...
		for (unsigned int j = 0u; j < latitude_vertices_count; ++j) {
			...
			// binormal
			binormals[index] = glm::vec3(radius * sin_theta * cos_phi,
										 radius * sin_phi,
                                    ...
                                    unsigned int const spread_split_count)
{
	...
	auto const circle_slice_vertices_count = circle_slice_edges_count + 2u;
	auto const spread_slice_vertices_count = spread_slice_edges_count + 2u;
