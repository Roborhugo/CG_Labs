...
int main()
{
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// nodes.
		//glm::mat4 earth_transforms = earth.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), glm::translate(glm::mat4(1.0f), glm::vec3(2.0f, 0.0f, 0.0f)), show_basis);
		//moon.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), earth_transforms, show_basis);

		//1.5
		std::stack<CelestialBodyRef> scene_graph;
		CelestialBodyRef root = { &sun, glm::translate(glm::mat4(1.0f), glm::vec3(2.0f, 0.0f, 0.0f)) };
		scene_graph.push(root);

		while (!scene_graph.empty()) {
			// Pop the scene graph
			CelestialBodyRef body_ref = scene_graph.top();
			scene_graph.pop();

			// Render body
			glm::mat4 body_transforms = body_ref.body->render(animation_delta_time_us,
				camera.GetWorldToClipMatrix(),
				body_ref.parent_transform,
				show_basis);

			// Push children to stack
			for (CelestialBody* bp : body_ref.body->get_children()) {
				scene_graph.push(CelestialBodyRef{ bp, body_transforms });
			}
		}
